...
class TestExpectedValueTestCases(unittest.TestCase):
    def test_good_ExpectedValueTest(self):
        evt = ExpectedValueTest(url="http://localhost:5000/v666/predict/",inputs={"a":5,"b":5}, predict_label="answer",
                                expected_output=10)
...
class TestFactoryTest(unittest.TestCase):
    ...
    def test_factory_success(self):
        evt = TestFactory.create("ExpectedValueTest", url="localhost/blah/predict/",inputs=[1,2,3],predict_label='predict', expected_output=6)
